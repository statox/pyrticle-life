...
def create_shared_memory_nparray(data):  # type: ignore[no-untyped-def]
...
def release_shared(name):  # type: ignore[no-untyped-def]
    shm = shared_memory.SharedMemory(name=name)
    shm.close()
    shm.unlink()  # Free and release the shared memory block


def np_sum(name, start, stop):  # type: ignore[no-untyped-def]
...
if __name__ == "__main__":
    shm = create_shared_memory_nparray(data)  # type: ignore[no-untyped-call]
    benchmark()  # type: ignore[no-untyped-call]
    release_shared(NP_SHARED_NAME)  # type: ignore[no-untyped-call]
